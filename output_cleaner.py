...
# Output cleaning utilities for model responses
...
class OutputCleanConfig:
    """Configuration for output cleaning"""
    mode: str = "prompt"
...
def clean_model_output(text: str, config: Optional[OutputCleanConfig] = None) -> str:
    """
    Clean model output by removing thinking blocks, code fences,
    role prefixes, and other artifacts.
    """
    if not text:
        return ""
...
def _extract_from_json(text: str, mode: str) -> Optional[str]:
    """Try to extract text content from JSON wrapper"""
...
def _drop_preamble(text: str) -> str:
    """Drop common preamble phrases from output"""
...
def _strip_planning_paragraphs(text: str) -> str:
    """Strip paragraphs that look like planning/reasoning"""
    paragraphs = [
        p.strip() 
        for p in re.split(r"\n\s*\n", (text or "").strip()) 
        if p.strip()
...
def clean_for_prompt_generation(text: str) -> str:
    """Clean output specifically for prompt generation use case"""
    return clean_model_output(text, OutputCleanConfig(
        mode="prompt",
        strip_think=True,
        strip_code_fences=True,
        strip_role_prefixes=True,
        strip_json_wrappers=True,
        strip_leading_preamble=True,
        strip_planning=True,
        keep_first_paragraph_only=False,
    ))


def clean_for_text_output(text: str) -> str:
    """Clean output for general text output"""
    return clean_model_output(text, OutputCleanConfig(
        mode="text",
        strip_think=True,
        strip_code_fences=True,
